...
export const tenant = pgTable(
  'Tenant',
  {
    ...
    status: varchar('status', { length: 20, enum: ['active', 'inactive'] })
      .notNull()
      .default('active'),
    settings: json('settings')
      ...
      .default({
        ...
        ai: {
          ...
          maxTokens: 2000,
          systemPrompt:
            'You are a helpful benefits advisor for employees. Be friendly, knowledgeable, and professional.',
          tone: 'professional',
...
export const tenantUser = pgTable(
  'TenantUser',
  {
    ...
    role: varchar('role', { length: 20, enum: ['owner', 'admin', 'member'] })
      .notNull()
      .default('member'),
  ...
  (table) => ({
    tenantUserIdx: uniqueIndex('tenant_user_idx').on(
      table.tenantId,
      table.userId,
    ),
...
export const tenantChat = pgTable(
  'TenantChat',
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    tenantId: uuid('tenantId')
  ...
  (table) => ({
    tenantChatIdx: uniqueIndex('tenant_chat_idx').on(
      table.tenantId,
      table.chatId,
    ),
...
export const userProfile = pgTable('UserProfile', {
  ...
  budgetPriority: varchar('budgetPriority', { length: 50 })
    .notNull()
    .default('low_premium'),
...
export const benefitsPlan = pgTable('BenefitsPlan', {
  ...
  monthlyPremium: json('monthlyPremium')
    .$type<{
      individual: number;
  ...
  copays: json('copays')
    ...
    .notNull(),
  prescriptionCoverage: json('prescriptionCoverage')
    .$type<{
      generic: number;
      brand: number;
      specialty: number;
